...
def avail_mem(convert: bool = False):
...
def total_mem(convert: bool = False):
    stat = StatFs(Environment.getDataDirectory().getPath())
...
def used_mem(convert: bool = False):
    stat = StatFs(Environment.getDataDirectory().getPath())
...
def avail_ram(convert: bool = False):
...
def total_ram(convert: bool = False):
    memInfo = MemoryInfo()
    service = activity.getSystemService(Context.ACTIVITY_SERVICE)
    service.getMemoryInfo(memInfo)
    if convert:
...
def used_ram(convert: bool = False):
    memInfo = MemoryInfo()
    service = activity.getSystemService(Context.ACTIVITY_SERVICE)
    service.getMemoryInfo(memInfo)
    if convert:
...
def device_info(text, convert=False):
    ...
    os = {
        ...
        'avail_mem': avail_mem(convert),
        'total_mem': total_mem(convert),
        'used_mem': used_mem(convert),
        'avail_ram': avail_ram(convert),
        'total_ram': total_ram(convert),
        'used_ram': used_ram(convert),
        'bat_level': bm.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY),
        'bat_capacity': round((count / cap) * 100),
        'bat_temperature': intent.getIntExtra(BatteryManager.EXTRA_TEMPERATURE, 0) / 10,
